...
{	
	...
	setup(&max_jit_peek_class, max_jit_peek_new, (method)max_jit_peek_free, (short)sizeof(t_max_jit_peek), 
		0L, A_GIMME, 0);
...
{	
	...
	x->vectors[0] = sp[x->dimcount]->s_vec;
	for (i=0;i<(x->dimcount);i++)	
		x->vectors[i+1] = sp[i]->s_vec;
